...
一份详细的海底地形图可以保障水下船体航行，同时在预测海洋灾害、探测海底资源等方面具有重要作用。测绘海底地形图有多种方法，其中多波束测深系统是一种用于测量海底地形的、承载多传感器的系统。它比单波束测深系统测量范围大、速度快、精度高，特别适合大面积海底地形的勘探。进行探测时，测线产生的相邻条带需要有 10%至20%的重叠率。重叠率过高会导致数据冗余、处理测量结果过程繁琐；重叠率过低会导致漏测。由于在实际探测中，海底地形较为复杂，不能简单地根据海区平均水深设计测线间隔。为了避免重叠率过高或过低的情况、提高探测效率和准确性，需要建立适当的数学模型，来设计合理的测线间隔。
...
$$
D_i = D_{i-1} + \Delta x \cdot \tan\alpha (4)
$$
...
$$
\frac{\sin\left( \frac{\pi - \theta}{2} - \alpha \right)}{D_{i\text{左}}} = \frac{\sin\frac{\theta}{2}}{\left( \frac{W_i}{2} \right)_{\text{左}}} (5)
$$
...
$$
\frac{\sin\left( \frac{\pi - \theta}{2} + \alpha \right)}{D_{i\text{右}}} = \frac{\sin\left( \frac{\theta}{2} \right)}{\left( \frac{W_i}{2} \right)_{\text{右}}} (6)
$$
...
$$
\left( \frac{W_i}{2} \right)_{\text{左}} + \left( \frac{W_i}{2} \right)_{\text{右}} = W_{i\text{总}} (7)
$$
...
$$
W_i = D_i \cdot \frac{\sin\theta}{2} \left[ \frac{1}{\sin\left( \frac{\pi - \theta}{2} + \alpha \right)} + \frac{1}{\sin\left( \frac{\pi - \theta}{2} - \alpha \right)} \right] (8)
$$
...
$$
d' = d \cdot \frac{\cos\left( \frac{\theta}{2} - \frac{\alpha}{2} \right)}{\cos\left( \frac{\theta}{2} - \alpha + \frac{\alpha}{2} \right)} (10)
$$
...
$$
\mathbf{A}_{\text{坡}} = \sqrt{\left(\frac{h}{d_{\text{水平}}}\right)^2 + \left(\frac{h}{d_{\text{水平}} \cdot \tan \theta}\right)^2} = \tan \alpha \cdot \cos \theta (11)
$$
...
$$
d_{\text{深}} = d_{\text{初}} - \Delta d \cdot \cos(\pi - \theta) \cdot \tan \alpha (12)
$$
...
$$
W_{\text{覆}} = d_{\text{深}} \cdot \tan \frac{\beta}{2} \left( \frac{1}{\tan(\alpha - \frac{\beta}{2})} - \tan \alpha + \frac{1}{\tan(\alpha + \frac{\beta}{2})} + \tan \alpha \right) (13)
$$
...
$$
(d_{\text{初}} - \Delta d \cdot \tan \alpha) \cdot \tan \frac{\beta}{2} \cdot \cos \theta = \eta \cdot W_{\text{测}}( \alpha - \frac{\beta}{2} - \theta) (15)
$$
...
$$
\Delta d = \frac{\cos \theta \cdot d_{\text{初}} \cdot \tan \frac{\beta}{2}}{\tan(\frac{\beta}{2} - \theta) + \tan \alpha \cdot \tan \frac{\beta}{2}} (16)
$$
...
$$
d_{\text{测1}} = d_{\text{初}} - \Delta d \cdot \tan \alpha (17)
$$
...
$$
\left( \frac{W_{\text{左}}}{2} \right) + \left( \frac{W_{\text{右}}}{2} \right) - \Delta d = W_{\text{重}} (18)
$$
...
$$
d_{\text{测}-1} \cdot \tan \frac{\beta}{2} \cdot \theta + d_{\text{测}} \cdot \tan \frac{\beta}{2} \cdot \theta - d_{\text{测}-1} - d_{\text{测}} \cdot \tan \alpha = \eta \cdot d_{\text{测}} \cdot \tan \frac{\beta}{2} \left( \frac{1}{\theta} + \frac{1}{\theta} \right) (20)
$$
...
$$
d_{\text{测}} = \frac{d_{\text{测}-1} \cdot \tan \frac{\beta}{2} \cdot \theta - 1 \cdot \tan \alpha \cdot \tan \frac{\beta}{2} \left( \frac{1}{\theta} + \frac{1}{\theta} \right) - \tan \frac{\beta}{2} \cdot \theta - 1}{\tan \alpha} (21)
$$
...
$$
\min(W_{\text{总}}) \quad \min(W_{\text{测}} - W) (22)
$$
...
$$
\min(W_{\text{总}}) \quad \min(W_{\text{测}} - W) \rightarrow \max(W(x_i, y_i)) \rightarrow \text{最佳航线方向} (23)
$$
...
在上图中，我们用圆锥模拟不同方向，用两类斜面去截圆锥，从其在水平面的投影来寻找最优的𝐀𝐀，可以看出它们的投影为类椭圆，总体仍然以长轴（即沿梯度为开角方向）取最大，但在极端情况下，可能会出现右侧中长轴变短的情况，不过该地形在本题地图中并未出现，且也可近似认为其类长轴为最宽处。关于重叠率，本题并未给出前几问中那样精确的范围，只需要不超过 20%，参考题干中的“为保证测量的便利性和数据的完整性，相邻条带之间应有10%至20%的重叠率”，我们以此作为标准。对于开角的确定，题目中并未给出数值上的参考。我们对开角𝐀在±30°内进行了覆宽变化的探究。在深度为 7 0 的情况下，做出如下三维图。
